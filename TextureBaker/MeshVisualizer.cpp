...
MeshVisualizer::MeshVisualizer(const std::string& path) {
  ...
  for(const tinyobj::shape_t& shape:reader.GetShapes()) { //shape
    ...
    for(GLuint fid=0,voff=0; fid<mesh.num_face_vertices.size(); fid++) { //mesh
      ...
      initializeComponent(path,component,reader.GetMaterials()[mesh.material_ids[fid]]);
      for(GLuint i=1; i<(GLuint)nV-1; i++) { //face
        for(GLuint vid: std::vector<GLuint>({0,i,i+1})) {  //triangle
          ...
          //Eigen::Matrix<GLfloat,3,1> c(reader.GetAttrib().colors[a.vertex_index*3+0],
          //                             reader.GetAttrib().colors[a.vertex_index*3+1],
          //                             reader.GetAttrib().colors[a.vertex_index*3+2]);
          ...
          //component._mesh->setColor(GL_TRIANGLES,c[0],c[1],c[2]);
        }
...
void MeshVisualizer::initializeComponent(const std::string& path,MeshComponent& component,const tinyobj::material_t& material) {
  if(!component._mesh) {
    ...
    //component._mesh->setColorAmbient(GL_TRIANGLES,material.ambient[0],material.ambient[1],material.ambient[2]);
    //component._mesh->setColorSpecular(GL_TRIANGLES,material.specular[0],material.specular[1],material.specular[2]);
  }
