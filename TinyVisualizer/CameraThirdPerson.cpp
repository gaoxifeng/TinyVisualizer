...
namespace DRAWER {
//Camera3D
    CameraThirdPerson::CameraThirdPerson(GLfloat angle,const Eigen::Matrix<GLfloat,3,1>& up)
...
void CameraThirdPerson::focusOn(std::shared_ptr<Shape> s) {
  _focus=s;
}
void CameraThirdPerson::mouse(GLFWwindow* wnd,int button,int action,int) {
...
void CameraThirdPerson::wheel(GLFWwindow*,double,double yoffset) {
  if(_speedMode)
    _speed*=std::pow(1.1,yoffset);
  else _sensitive*=std::pow(1.1,yoffset);
}
void CameraThirdPerson::motion(GLFWwindow* wnd,double x,double y) {
  if(_inMotion) {
    _xCurr=x;
    _yCurr=y;
  }
}
void CameraThirdPerson::frame(GLFWwindow* wnd,GLfloat time) {
  if(_inMotion) {
...
void CameraThirdPerson::key(GLFWwindow* wnd,int key,int scan,int action,int mods) {
  if(key==GLFW_KEY_W)
...
void CameraThirdPerson::draw(GLFWwindow* wnd,const Eigen::Matrix<GLfloat,6,1>& bb) {
...
Eigen::Matrix<GLfloat,-1,1> CameraThirdPerson::getCameraRay(GLFWwindow* wnd,double x,double y) const {
...
Eigen::Matrix<GLfloat,-1,1> CameraThirdPerson::getViewFrustum() const {
  return getViewFrustum3DPlanes();
}
void CameraThirdPerson::begin(GLfloat& theta,GLfloat& phi) const {
...
void CameraThirdPerson::end(GLfloat theta,GLfloat phi) {
...
GLfloat CameraThirdPerson::clampMin(GLfloat val) {
  static GLfloat minVal=3;
  if(val>0)
    return (val<minVal)?0:val;
  else return (val>-minVal)?0:val;
}
void CameraThirdPerson::setPosition(const Eigen::Matrix<GLfloat,3,1>& pos) {
  _pos=pos;
}
void CameraThirdPerson::setDirection(const Eigen::Matrix<GLfloat,3,1>& dir) {
  _dir=dir.normalized();
}
void CameraThirdPerson::setUp(const Eigen::Matrix<GLfloat,3,1>& up) {
  _up=up.normalized();
}
}
