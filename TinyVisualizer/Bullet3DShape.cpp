...
namespace DRAWER {
Bullet3DShape::Bullet3DShape()
    : _body(NULL), _localTrans(Eigen::Matrix<GLfloat, 4, 4>::Identity()) {}
#ifdef BULLET_SUPPORT
Bullet3DShape::Bullet3DShape(const btCollisionObject *b,
                             std::shared_ptr<Texture> tex)
    : _body(b), _localTrans(Eigen::Matrix<GLfloat, 4, 4>::Identity()) {
  createShape(b, tex);
}
void Bullet3DShape::syncWorld(std::shared_ptr<SceneNode> &root,
                              const btDiscreteDynamicsWorld *world,
                              std::shared_ptr<Texture> tex,
                              const Eigen::Matrix<GLfloat, 3, 1> &c,
                              const Eigen::Matrix<GLfloat, 3, 1> &cB,
                              GLfloat lineWidth) {
  // objects in shapes
...
Eigen::Matrix<GLfloat, 6, 1> Bullet3DShape::getBB() const {
  ...
  if (_body) {
    const btTransform &t = _body->getWorldTransform();
    for (GLfloat x : {retL[0], retL[3]})
      for (GLfloat y : {retL[1], retL[4]})
        for (GLfloat z : {retL[2], retL[5]}) {
          btVector3 pos = t * btVector3(x, y, z);
          ret = unionBB(
              ret, Eigen::Matrix<GLfloat, 3, 1>(pos.x(), pos.y(), pos.z()));
        }
  } else
    ret = retL;
#else
...
bool Bullet3DShape::rayIntersect(const Eigen::Matrix<GLfloat, 6, 1> &ray,
                                 GLfloat &alpha) const {
  Eigen::Matrix<GLfloat, 6, 1> rayLocal;
  Eigen::Matrix<GLfloat, 3, 3> invR = _localTrans.block<3, 3>(0, 0).inverse();
  rayLocal.segment<3>(0) =
      invR * (ray.segment<3>(0) - _localTrans.block<3, 1>(0, 3));
  rayLocal.segment<3>(3) = invR * ray.segment<3>(3);
  return CompositeShape::rayIntersect(rayLocal, alpha);
}
void Bullet3DShape::setLocalTransform(
    const Eigen::Matrix<GLfloat, 4, 4> &localTrans) {
  _localTrans = localTrans;
}
void Bullet3DShape::setLocalTranslate(
    const Eigen::Matrix<GLfloat, 3, 1> &localTrans) {
  _localTrans.block<3, 1>(0, 3) = localTrans;
}
void Bullet3DShape::setLocalRotate(
    const Eigen::Matrix<GLfloat, 3, 3> &localTrans) {
  _localTrans.block<3, 3>(0, 0) = localTrans;
}
Eigen::Matrix<GLfloat, 4, 4> Bullet3DShape::getLocalTransform() const {
  return _localTrans;
}
const btCollisionObject *Bullet3DShape::getBody() const { return _body; }
#ifdef BULLET_SUPPORT
void Bullet3DShape::createShape(const btCollisionObject *b,
                                std::shared_ptr<Texture> tex, int RES) {
  ...
  if (shape->getShapeType() == BOX_SHAPE_PROXYTYPE) {
    ...
    btVector3 he =
        static_cast<const btBoxShape *>(shape)->getHalfExtentsWithMargin();
    _localTrans.block<3, 3>(0, 0).diagonal() =
        Eigen::Matrix<GLfloat, 3, 1>(he.x(), he.y(), he.z());
    addShape(_fillBox);
    if (!tex)
      addShape(_borderBox);
    ...
    GLfloat rad = static_cast<const btSphereShape *>(shape)->getRadius();
    ...
    GLfloat rad = static_cast<const btCapsuleShape *>(shape)->getRadius();
    GLfloat height =
        static_cast<const btCapsuleShape *>(shape)->getHalfHeight() / rad;
