...
void ImGuiPlugin::init(GLFWwindow* window) {
  ...
  if (window) {
    ...
    if(!_context) {
      ...
      static ImGuiContext*globalContext=ImGui::CreateContext();
...
void ImGuiPlugin::reloadFont(int font_size) {
  ...
  io.Fonts->Clear();
  io.Fonts->AddFontFromMemoryCompressedTTF(droid_sans_compressed_data,droid_sans_compressed_size,font_size*_hidpiScaling);
  io.FontGlobalScale=1.0/_pixelRatio;
}
...
float ImGuiPlugin::menuScaling() {
  return _hidpiScaling/_pixelRatio;
}
}
