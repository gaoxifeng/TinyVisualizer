const std::string LightFragConditional=
  ...
  "  float closestDepth=texture(lights[lid].depthMap,v0-l).r*far_plane;\n"
  ...
  "    for(int i=0;i<27;++i) {\n"
  "      float closestDepth=texture(lights[lid].depthMap,dir+gridSamplingDisk[i]*invShadowMapSize*pass).r*far_plane;   // undo mapping [0;1]\n"
  "      if(currentDepth-bias>closestDepth)\n"
  "        shadow+=1.0;\n"
  "    }\n"
  ...
  "{\n"
  "  vec3 N=normalize(vN);\n"
  "  vec4 colorDiffuse=texture(diffuseMap,tc);\n"
  "  vec4 colorSpecular=texture(specularMap,tc);\n"
  ...
  "    vec4 Iamb=lights[i].ambient;\n"
  "    //calculate Diffuse Term:\n"
  "    vec4 Idiff=lights[i].diffuse*colorDiffuse*max(dot(N,L),0.0);\n"
  "    Idiff=clamp(Idiff,0.0,1.0);\n"
  "    //calculate Specular Term:\n"
  "    vec4 Ispec=lights[i].specular*colorSpecular*pow(max(dot(R,E),0.0),lights[i].shininess);\n"
